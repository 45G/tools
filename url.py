...
#    html = response.read(int(args.length))
    ...
    print time.time(), "Bytes:", int(args.length), "ms:", (after-before).seconds*1000.0 + (after-before).microseconds/1000.0, \
	"Mbps:",  int(args.length)*8/( (after-before).seconds*1000000.0 + (after-before).microseconds), "wlan_rx:", after_wlan - before_wlan, "4g_rx:", after_4g - before_4g     
    sys.stdout.flush()
    i = i + 1
